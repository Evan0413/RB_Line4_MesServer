...
class Broker:
        ...
        while connected:
            ...
            try:
                ...
                if wait_deliver in done:
                    ...
                    print(remote_address)
                    print(remote_port)
                    print(app_message.data.decode('utf-8'))
                    Recv(remote_address, app_message.data.decode('utf-8'))
        ...
        for k_filter in self._subscriptions:
            ...
            for (target_session, qos) in subscriptions:
                ...
                if cache.get('EquipConnect', default=None) is not None:
                    print(cache.get('EquipConnect')[str(target_session.client_id)])
                else:
    ...
    async def publish_session_retained_messages(self, session):
        ...
        while not session.retained_messages.empty():
            ...
            # print("##publish_session_retained_messages:"+str(retained.data))
