    ...
    try:
        ...
        if command == "0x02":   # SP
            ...
            try:
                ...
                # 借用项目须同时匹配第二个下划线前的前缀和型号最后一位，例如：
                # C06_SVC_Q_P01_4_YQ 只能匹配同样以 C06_SVC 开头、Q 结尾的本体型号。
                ...
                if body_model != stand_model:
                    ...
                    body_last_char = body_model[-1] if body_model else ""
                    stand_last_char = stand_model[-1] if stand_model else ""
                    if body_prefix == "" or body_prefix != stand_prefix or body_last_char != stand_last_char:
                        msg = "主码型号(" + body_model + ")与生产型号(" + stand_model + ")不匹配"
                        raise Exception(msg)
                    log.info("借用项目型号匹配: 前缀=" + body_prefix + ", 最后一位=" + body_last_char)
