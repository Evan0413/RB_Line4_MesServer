    ...
    try:
        ...
        if command == "0x02":   # SP
            ...
            try:
                ...
                # 借用项目须匹配第三个下划线前的前缀，例如：
                # C06_SVC_Q_P01_4_YQ 只能匹配同样以 C06_SVC_Q 开头的本体型号。
                ...
                if body_model != stand_model:
                    # 型号允许使用项目型号或完整型号：任一方包含另一方即可。
                    if body_model not in stand_model and stand_model not in body_model:
                        msg = "主码型号(" + body_model + ")与生产型号(" + stand_model + ")不匹配"
                        raise Exception(msg)
                    log.info("型号包含关系匹配: 主码型号=" + body_model + ", 生产型号=" + stand_model)
                ...
                if select_count == 0:               #如果为第一站 只插入
                    ...
                    if len(data) > 0:
                        ...
                        if ordersn_data[0]['current_station'] == stno.upper():
                            ...
                            # 产品到达ST04及以后，不允许回退到任何前序站位重投。
                            if current_stnum >= 4 and stnum < current_stnum:
                                result = "NG"
                                msg = "当前产品已到达ST" + str(current_stnum) + "，ST04及以后不允许回退重投到ST" + str(stnum)
                                log.warning(msg)
                                raise Exception(msg)
