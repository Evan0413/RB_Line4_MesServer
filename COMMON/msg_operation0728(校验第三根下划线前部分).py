        ...
        if command == "0x02":   # SP
            ...
            try:
                ...
                if body_model != stand_model:
                    ...
                    body_prefix = "_".join(body_parts[:3]) if len(body_parts) >= 3 else ""
                    stand_prefix = "_".join(stand_parts[:3]) if len(stand_parts) >= 3 else ""
                    if body_prefix == "" or body_prefix != stand_prefix:
                        msg = "主码型号(" + body_model + ")与生产型号(" + stand_model + ")不匹配"
                        raise Exception(msg)
                    log.info("借用项目型号匹配: 第三根下划线前前缀=" + body_prefix)
