    ...
    try:
        ...
        if command == "0x02":   # SP
            ...
            try:
                ...
                # if not len(qr_confdata) == 0:
                #     qr_model = qr_confdata[0]['gp_model']
                #     if not qr_model.upper() == gp_model.upper():
                #         if qr_model.upper() in gp_model.upper() or gp_model.upper() in qr_model.upper():
                #             if qr_model.upper() == gp_model.rsplit("_", 1)[0].upper():
                #                 msg = "支架型号包含本体型号 前缀匹配01"
                #             else:
                #                 msg = "本体型号包含支架型号 前缀匹配02"
                #         else:
                #             msg = "主码型号与生产型号不匹配"
                #             raise Exception(msg)
