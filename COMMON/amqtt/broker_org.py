...
class Broker:
    ...
    async def django_internal_message_broadcast(self, topic, data, qos=None):
        log.info("--------django_internal_message_broadcast" + str(data) + str(topic))
    ...
    async def client_connected(
        ...
        try:
            ...
            server.release_connection()
            return
        if client_session.clean_session:                                                 #session保留功能
        ...
        while connected:
            try:
                ...
                if disconnect_waiter in done:
                    log.info("-----------断联部分处理---------" + client_session.client_id + "----" + remote_address)
                    ...
                    #存入cache
                    Conn(client_session.client_id, remote_address, remote_port, False)
                    if result is None:
    ...
    async def _broadcast_loop(self):
        ...
        try:
            while True:
                # broker主循环中 对django回复的消息队列进行扫描
                if not len(reply_msgs) == 0:
                    while not len(reply_msgs) == 0:
                        d_msg = reply_msgs.popleft()
        ...
        for k_filter in self._subscriptions:
            ...
            for (target_session, qos) in subscriptions:
                ...
                if target_session.transitions.state != "connected":
                    log.info("---------disconnect _retain_broadcast_message---------" + str(broadcast["data"]))
                ...
                # Broker会遍历订阅表，对当前topic下用户广播，不对消息源区别发送，想只发不收可以不订阅只发送
                handler = self._get_handler(target_session)  #Session(clientId=, state=)
                # 创建一个对频道的发送事件
